...
#### 访问 __weak 修饰的变量，是否已经被注册在了 @autoreleasePool 中?为什 么?
...
weak_table_t:一个结构体，里面是一个数组和维护这个数组大小的值。数组里存的又是结构体，这个结构体里存的是对象的弱引用的地址，当对象释放后，这个数组（referrers）里面的对象全部置为nil
https://www.jianshu.com/p/ef6d9bf8fe59
