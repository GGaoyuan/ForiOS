...
面试总结
http://www.cocoachina.com/articles/899939?filter=rec
http://www.cocoachina.com/articles/900215?filter=rec

缓存策略
https://www.jianshu.com/p/3124e4c125cc

SD缓存策略
https://blog.csdn.net/u013602835/article/details/70228555
...
def get_max_str(str):
    ...
    for i in range(0,count+1):
        for j in range(0,i):
            sub_str = str[j:i]
            res = isHuiWen(sub_str)
            if res and len(sub_str)>len(res_str):
                res_str = sub_str
    return res_str
while True:
    a = input()
